...
static void vhost_vfio_plat_get_config(VirtIODevice *vdev, uint8_t *config_data)
{
...
static void vhost_vfio_plat_set_config(VirtIODevice *vdev,
                                      const uint8_t *config_data)
{
}

static uint64_t vhost_vfio_plat_get_features(VirtIODevice *vdev, uint64_t f,
                                            Error **errp)
{
    VhostVFIOPlat *s = VHOST_VFIO_PLAT(vdev);
    f |= s->host_features;
    ...
    error_report("@@@@@@@@@@@@ %s get_features 0x%lx\n", __func__, (long)f);
    return f;
}
...
static void vhost_vfio_plat_device_realize(DeviceState *dev, Error **errp)
{
    ...
    virtio_init(vdev, "vhost-vfio", VIRTIO_ID_VFIO, 0);
    ...
    /* Combine vhost VFIO with real VFIO device */
    vfio_dev = VFIO_PLATFORM_DEVICE(s->vfio_dev);
    info.consumer_fd = vfio_dev->vbasedev.fd;
    info.companion_index = s->index;
...
//static const VMStateDescription vmstate_vhost_vfio_plat = {
//    .name = "vhost-vfio-plat",
...
//    dc->vmsd = &vmstate_vhost_vfio_plat;
    ...
    vdc->get_config = vhost_vfio_plat_get_config;
    vdc->set_config = vhost_vfio_plat_set_config;
